...
import { renderNativeAudioTrim } from './native-audio-trim.js';
...
export class MediaRenderRunner {
  ...
  async runVideoEdits(source, edits, { signal, onState, onProgress } = {}) {
    if (this.#active) {
      return { ok: false, state: 'failed', error: safeError('Another media-processing job is already running.', 'JOB_BUSY') };
    }
    ...
    const rendered = await renderNativeVideoTrim(source, edits, {
      signal,
    ...
    const validation = await validateRenderOutput(rendered.blob, source, {
      expected: {
        ...
        duration: rendered.settings?.outputDuration ?? rendered.range.duration,
        width: rendered.settings?.outputWidth ?? source.width,
        height: rendered.settings?.outputHeight ?? source.height
      }
    });
  ...
  async runVideoTrim(source, trim, options = {}) {
    return this.runVideoEdits(source, trim, options);
  }

  async runAudioEdits(source, edits, { signal, onState, onProgress } = {}) {
    if (this.#active) {
      return { ok: false, state: 'failed', error: safeError('Another media-processing job is already running.', 'JOB_BUSY') };
    }
    ...
    if (source?.mediaType !== 'audio') {
      return { ok: false, state: 'failed', error: safeError('Real audio trim rendering requires a local audio source.', 'UNSUPPORTED_MEDIA') };
    }
    ...
    emitState('running', 'Starting browser audio encoder');
    const rendered = await renderNativeAudioTrim(source, edits, {
      signal,
      onStage: (stage) => emitState('running', stage),
      onProgress: emitProgress
    });
    ...
    if (!rendered.ok) {
      emitState(rendered.state || 'failed', rendered.error?.message || 'Audio trim failed');
      this.#active = null;
      return rendered;
    }

    emitState('validating', 'Validating trimmed audio');
    emitProgress(99, 'Validating trimmed audio');
    const validation = await validateRenderOutput(rendered.blob, source, {
      expected: {
        mime: rendered.blob.type,
  ...
  async runAudioTrim(source, trim, options = {}) {
    return this.runAudioEdits(source, trim, options);
  }
