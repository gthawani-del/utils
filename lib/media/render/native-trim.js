...
export async function renderNativeVideoTrim(source, trim, {
  signal,
  onProgress,
  onStage
} = {}) {
  ...
  const range = normalizeVideoTrimRange(trim?.start, trim?.end, source?.duration);
  if (range.duration <= 0) {
  ...
  try {
    ...
    if (!stream || stream.getVideoTracks().length === 0) {
      cleanup();
      return { ok: false, state: 'unsupported', error: { code: 'CAPTURE_UNAVAILABLE', message: 'Browser could not expose the video stream for local trimming.' } };
    }

    const chunks = [];
    recorder = new MediaRecorder(stream, { mimeType: support.mime });
    ...
    const result = await new Promise((resolve) => {
      ...
      interval = setInterval(() => {
        ...
        onProgress?.(Math.max(0, Math.min(99, progress)), 'Encoding trim');
      ...
      }, videoTrimTimeoutMs(range.duration));
