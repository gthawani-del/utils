...
export function videoOutputDimensions(width, height, aspect = 'original') {
  const rawWidth = Math.max(1, Math.floor(finite(width, 1)));
  const rawHeight = Math.max(1, Math.floor(finite(height, 1)));
  const sourceWidth = evenFloor(rawWidth);
  const sourceHeight = evenFloor(rawHeight);
  const normalizedAspect = ALLOWED_OUTPUT_ASPECTS.includes(String(aspect)) ? String(aspect) : 'original';
  if (normalizedAspect === 'original') {
    return { width: rawWidth, height: rawHeight, aspect: 'original', changed: false };
  }
