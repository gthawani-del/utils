...
export async function renderNativeAudioTrim(source, trim, {
  signal,
  onProgress,
  onStage
} = {}) {
  ...
  const range = normalizeAudioTrimRange(trim?.start, trim?.end, source?.duration);
  if (range.duration <= 0) {
  ...
  try {
    ...
    const result = await new Promise((resolve) => {
      ...
      interval = setInterval(() => {
        ...
        onProgress?.(Math.max(0, Math.min(99, progress)), 'Encoding audio trim');
    ...
    return {
      ...
      error: {
        ...
        message: error instanceof Error ? error.message : 'Audio trim failed safely.'
      }
    };
  } finally {
    await cleanup();
