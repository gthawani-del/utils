...
export async function validateRenderOutput(blob, source, {
  timeoutMs = MEDIA_RENDER_BUDGET.validationTimeoutMs,
  expected = null
} = {}) {
  ...
  try {
    ...
    const expectedDuration = Number(expectation?.duration);
    ...
    if (mediaType === 'video') {
      const expectedWidth = Number(expectation?.width);
      const expectedHeight = Number(expectation?.height);
