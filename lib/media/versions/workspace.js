...
export function initVersionWorkspace({
  ...
  async function createTrimVersion() {
    ...
    if (!isAudio && !trimChanged && !muted) {
      setStatus('Adjust trim or enable Mute output before creating a video Version.');
      return;
    }
    ...
    setStatus(isAudio
      ...
      : `Rendering ${pending.name} locally with trim and mute settings. Playback-speed export and resize remain for later Step 6 sub-steps.`);
