...
export function initVersionWorkspace({
  ...
  function renderOriginal() {
    ...
    if (getSource()?.kind === 'local-file' && getSource()?.mediaType === 'video' && getSource()?.objectUrl) {
      const video = document.createElement('video');
      video.controls = true;
      video.preload = 'metadata';
      video.src = getSource().objectUrl;
      video.setAttribute('aria-label', 'Original video');
      originalPlayer.append(video);
    }
  ...
  function renderResult() {
    ...
    if (active.status === 'ready' && url) {
      ...
      video.preload = 'metadata';
      video.src = url;
      video.setAttribute('aria-label', active.name + ' rendered result');
      resultPlayer.append(video);
  ...
  function render() {
    ...
    createButton.disabled = getSource()?.kind !== 'local-file' || getSource()?.mediaType !== 'video';
  ...
  function versionName(index, start, end) {
  ...
  async function createTrimVersion() {
    const source = getSource();
    if (!source || source.kind !== 'local-file' || source.mediaType !== 'video' || !(source.file instanceof Blob)) {
      setStatus('Create Version currently requires a relinked local video source.');
      return;
    }

    const edits = getVideoEdits?.();
    ...
    if (start <= 0.001 && duration > 0 && Math.abs(end - duration) <= 0.05) {
      setStatus('Adjust the In or Out point first. Step 3 creates real trimmed Versions only.');
      return;
    }
    ...
    const pending = {
      ...
      status: 'rendering',
      blobRef: null,
      name: versionName(index, start, end),
    ...
    setStatus(`Rendering ${pending.name} locally. Preview speed is not included; Step 3 applies trim only.`);

    const result = await renderRunner.runVideoTrim(source, { start, end }, {
      signal: renderAbort.signal,
      onState: (job) => setRenderUi(job.status, job.progress, job.stage),
      onProgress: (job) => setRenderUi('running', job.progress, job.stage)
    });
    ...
    const outputFormat = mime.includes('webm') ? 'webm' : mime.includes('mp4') ? 'mp4' : mime || 'video';
