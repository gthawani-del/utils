...
export function initVersionWorkspace({
  ...
  async function createTrimVersion() {
    ...
    if (!isAudio && !trimChanged && !muted && !speedChanged) {
      setStatus('Adjust trim, playback speed or Mute output before creating a video Version.');
      return;
    }
    ...
    setStatus(isAudio
      ...
      : `Rendering ${pending.name} locally with trim, playback-speed and mute settings. Resize/aspect conversion remains for the next Step 6 sub-step.`);
