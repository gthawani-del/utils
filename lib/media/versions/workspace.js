...
export function initVersionWorkspace({
  ...
  function setupComparisonPlayback(active) {
    ...
    for (const [from, to, fromDuration, toDuration] of pair) {
      ...
      bindCompareEvent(from, 'seeked', () => moveEquivalent(from, to, fromDuration, toDuration));
    }
  }
  ...
  function switchAB(nextSide) {
    ...
    try { to.currentTime = equivalentCompareTime(from.currentTime, fromDuration, toDuration); } catch {}
    abSide = nextSide;
