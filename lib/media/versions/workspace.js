...
export function initVersionWorkspace({
  ...
  function renderCompareControls() {
    ...
    if (!canAB && compareMode === 'ab') compareMode = 'side-by-side';
    ...
    for (const button of mobileButtons) {
      ...
      button.textContent = view === 'original'
        ? (reference.type === 'parent' ? 'Parent' : 'Original')
        : 'Result';
  ...
  async function createTrimVersion() {
    ...
    if (!source || source.kind !== 'local-file' || !['video', 'audio'].includes(source.mediaType) || !(source.file instanceof Blob)) {
      const reason = state().baseVersionId === ORIGINAL_VERSION_ID
    ...
    if (!Number.isFinite(end) || end <= start) {
      const reason = 'Set a valid In/Out range before creating a Version.';
      setStatus(reason);
      return { ok: false, reason };
    }
    ...
    if (!isAudio && !trimChanged && !muted && !speedChanged && !aspectChanged) {
      const reason = 'Adjust trim, playback speed, output aspect or Mute output before creating a video Version.';
      setStatus(reason);
      return { ok: false, reason };
    }
    if (isAudio && !trimChanged && Math.abs(volume - 1) < 0.0001 && fadeIn <= 0 && fadeOut <= 0) {
      const reason = 'Adjust trim, volume, fade-in or fade-out before creating an audio Version.';
      setStatus(reason);
      return { ok: false, reason };
    }
    ...
    if (!result.ok) {
      ...
      const reason = result.error?.message || 'Render did not complete.';
      setStatus(`${pending.name} failed safely: ${reason} Original and Preview state are unchanged.`);
      render();
      return { ok: false, reason };
    }
  ...
  return {
    refresh: render,
    createVersion: createTrimVersion,
    onSourceChanged() {
      render();
    },
    resetForNewSource() {
