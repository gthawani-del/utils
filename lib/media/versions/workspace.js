...
export function initVersionWorkspace({
  ...
  async function createTrimVersion() {
    ...
    if (start <= 0.001 && duration > 0 && Math.abs(end - duration) <= 0.05) {
      setStatus(`Adjust the In or Out point first. Step 4 creates real ${isAudio ? 'audio' : 'video'} trimmed Versions only.`);
      return;
    }
    ...
    setStatus(isAudio
      ? `Rendering ${pending.name} locally. Step 4 applies trim only; volume and fades remain Preview until Step 5.`
      : `Rendering ${pending.name} locally. Preview speed is not included; Step 3 applies trim only.`);

    const runner = isAudio ? renderRunner.runAudioTrim.bind(renderRunner) : renderRunner.runVideoTrim.bind(renderRunner);
    const result = await runner(source, { start, end }, {
      signal: renderAbort.signal,
      onState: (job) => setRenderUi(job.status, job.progress, job.stage),
      onProgress: (job) => setRenderUi('running', job.progress, job.stage)
    });
