...
export function initAudioVideoWorkspace({
  ...
  const title = document.querySelector('#audio-video-title');
  ...
  async function ensurePreviewGraph() {
    ...
    if (previewPlayer === player && previewContext && previewAnalyser) {
    ...
    try {
      ...
      previewSourceNode = previewContext.createMediaElementSource(player);
      ...
      previewAnalyser.connect(previewContext.destination);
