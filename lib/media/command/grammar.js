...
function executionFor(type, project, status = 'ready') {
  if (status !== 'ready') return 'blocked';
  const source = sourceContext(project);
  if (type === 'trim' && source.local && ['audio', 'video'].includes(source.mediaType)) return 'renderable';
  if (['volume', 'fade-in', 'fade-out'].includes(type) && source.local && source.mediaType === 'audio') return 'renderable';
  if (type === 'set-aspect' && source.local && source.mediaType === 'video') return 'renderable';
  return 'workflow';
}
...
function statusFor(type, project) {
  ...
  if (type === 'set-aspect') {
    if (source.local && source.mediaType === 'video') {
      return { status: 'ready', reason: 'Renderable: uses the Version pipeline safe aspect-fit transform.' };
    }
    if (source.local && source.mediaType === 'audio') {
      return { status: 'ready', reason: 'Working workflow action: configures Audio → Video, but it does not create a Media Version.' };
    }
    return { status: 'blocked', reason: 'Output aspect requires a relinked local audio or video source.' };
  }
...
export function parseCommand(command, project = {}) {
  ...
  if (qcFix) {
    ...
    const qc = occurrence(text, /\b(?:run|check|perform)\s+(?:a\s+)?(?:qc|quality\s+control|forensics)\b/i);
    if (qc) {
  ...
  const explicitOpen = /\b(?:open|go\s+to|show|switch\s+to)\s+(.+?)(?=\s+(?:then|and)\s+|,|$)/gi;
  let openMatch;
  ...
  const ordered = dedupeAndSort(actions).map((item) => ({
    ...item,
    execution: executionFor(item.type, project, item.status)
  }));
  return {
    command: raw,
    actions: ordered,
...
export function reevaluateAction(input, project = {}) {
  ...
  if (['remove-silence', 'normalize-audio', 'generate-captions', 'target-size', 'encode-mp3'].includes(item.type)) {
    return { ...item, execution: 'blocked' };
  }
  ...
  if (item.type === 'trim') {
    ...
    if (start < 0 || end < start) {
      return { ...item, status: 'blocked', reason: 'Trim needs a valid start and end time, with end ≥ start.', execution: 'blocked' };
    }
  }

  if (item.type === 'volume') {
    const value = finite(item.params.percent, -1);
    if (value < 0 || value > 100) return { ...item, status: 'blocked', reason: 'Volume must be between 0% and 100%.', execution: 'blocked' };
  }

  if (item.type === 'fade-in' || item.type === 'fade-out') {
    if (finite(item.params.seconds, -1) < 0) return { ...item, status: 'blocked', reason: 'Fade duration must be zero or greater.', execution: 'blocked' };
  }
  ...
  return {
    ...
    status: state.status,
    reason: state.reason,
    execution: executionFor(item.type, project, state.status)
  };
}

export function planCapabilities(actions = []) {
  const items = Array.isArray(actions) ? actions : [];
  const renderable = items.filter((item) => item?.status === 'ready' && item?.execution === 'renderable').length;
  const workflow = items.filter((item) => item?.status === 'ready' && item?.execution !== 'renderable').length;
  const blocked = items.filter((item) => item?.status !== 'ready' || item?.execution === 'blocked').length;
  return {
    total: items.length,
    renderable,
    workflow,
    blocked,
    canPreview: items.length > 0 && blocked === 0,
    canCreateVersion: renderable > 0 && blocked === 0
  };
}


const RECIPE_ACTION_TYPES = new Set([
  'open-category',
  'run-qc',
  'apply-qc-fixes',
  'trim',
...
function recipeAdjustable(type) {
  if (type === 'trim') return [
    { key: 'start', label: 'Start (seconds)', type: 'number', min: 0, step: 0.01 },
    { key: 'end', label: 'End (seconds)', type: 'number', min: 0, step: 0.01 }
  ];
  if (type === 'volume') return [{ key: 'percent', label: 'Volume %', type: 'number', min: 0, max: 100, step: 1 }];
  if (type === 'fade-in' || type === 'fade-out') return [{ key: 'seconds', label: 'Seconds', type: 'number', min: 0, step: 0.1 }];
  if (type === 'set-aspect') return [{ key: 'aspect', label: 'Aspect', type: 'select', options: ['16:9', '9:16', '1:1'] }];
  if (type === 'target-size') return [{ key: 'megabytes', label: 'Target MB', type: 'number', min: 1, step: 1 }];
  if (type === 'open-category') return [{ key: 'category', label: 'Workspace', type: 'select', options: CATEGORY_ALIASES.map((item) => item.id) }];
  return [];
}

function recipeLabel(type, params) {
  if (type === 'open-category') {
    const category = CATEGORY_ALIASES.find((item) => item.id === params.category);
    return `Open ${category?.label || 'workspace'}`;
  }
  if (type === 'run-qc') return 'Run QC report';
  if (type === 'apply-qc-fixes') return 'Apply deterministic QC fixes';
  if (type === 'trim') return 'Trim media range';
  if (type === 'volume') return 'Set audio volume';
  if (type === 'fade-in') return 'Set fade in';
  if (type === 'fade-out') return 'Set fade out';
  if (type === 'set-aspect') return `Set output aspect to ${params.aspect || '16:9'}`;
  if (type === 'remove-silence') return 'Remove silence';
  if (type === 'normalize-audio') return 'Normalize audio / dialogue';
  if (type === 'generate-captions') return 'Generate captions / transcript';
  if (type === 'target-size') return `Target export size ≤ ${params.megabytes || 25} MB`;
  if (type === 'encode-mp3') return 'Encode podcast MP3';
  return type;
}

function recipeParams(type, params = {}) {
  if (type === 'trim') return { start: finite(params.start, 0), end: finite(params.end, 0) };
  if (type === 'volume') return { percent: finite(params.percent, 100) };
  if (type === 'fade-in' || type === 'fade-out') return { seconds: finite(params.seconds, 0) };
  if (type === 'set-aspect') {
    const aspect = ['16:9', '9:16', '1:1'].includes(params.aspect) ? params.aspect : '16:9';
    return { aspect };
  }
  if (type === 'target-size') return { megabytes: Math.max(1, finite(params.megabytes, 25)) };
  if (type === 'open-category') {
    const category = CATEGORY_ALIASES.some((item) => item.id === params.category) ? params.category : 'video';
    return { category };
  }
  return {};
}

export function actionSpecsFromPlan(actions) {
  return (Array.isArray(actions) ? actions : [])
    .filter((item) => RECIPE_ACTION_TYPES.has(item?.type))
    .slice(0, 30)
    .map((item) => ({
      type: item.type,
      params: recipeParams(item.type, item.params)
    }));
}

export function planFromActionSpecs(specs, project = {}) {
  const actions = [];
  for (const [index, spec] of (Array.isArray(specs) ? specs : []).slice(0, 30).entries()) {
    const type = String(spec?.type || '');
    if (!RECIPE_ACTION_TYPES.has(type)) continue;
    const params = recipeParams(type, spec?.params);
    let status = 'ready';
    let reason = '';

    if (['remove-silence', 'normalize-audio', 'generate-captions', 'target-size', 'encode-mp3'].includes(type)) {
      const reasons = {
        'remove-silence': 'Silence detection/removal is not implemented yet; the Command Assistant will not invent cuts.',
        'normalize-audio': 'Loudness normalization requires a real measurement/processing engine that is not enabled yet.',
        'generate-captions': 'Automatic speech-to-text is not enabled. Existing SRT/VTT import and manual cue editing remain available.',
        'target-size': 'Target-size encoding requires the future deterministic media encoder.',
        'encode-mp3': 'MP3 encoding is not enabled without a dedicated local encoder.'
      };
      status = 'blocked';
      reason = reasons[type];
    } else {
      const current = statusFor(type, project);
    ...
    const hydrated = action(
      `recipe-${index + 1}`,
      type,
      recipeLabel(type, params),
      index,
      params,
      recipeAdjustable(type),
      status,
      reason
    );
    actions.push(reevaluateAction(hydrated, project));
  }
  return actions.map((item, index) => ({ ...item, order: index }));
}
